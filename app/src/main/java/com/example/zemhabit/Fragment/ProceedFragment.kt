 ...
 * Use the [ProceedFragment.newInstance] factory method to
...
class ProceedFragment : Fragment() {
    ...
    init {
        Log.e("ProccedFragment", "In ProccedFragment")
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        activityresultlauncer = registerForActivityResult(
            ...
            if (it.resultCode == Activity.RESULT_OK) {
                Log.e("ProceedFragment()", "Sign OK")
                val data: Intent? = it.data
                val text: String? = data?.getStringExtra("key2")
                Log.e("ActivityResultLauncher", "Get Data = ${text}")
                if (text == "완료요청") {
                    val tab: TabLayout.Tab? = parentFragmentManager.fragments[0].view?.findViewById<TabLayout>(R.id.tl_habit)?.getTabAt(1)
                    tab?.select()

                    HabitFragment().refreshFragment(CompletionFragment(), childFragmentManager)
                } else if (text == "수정대기") {
                    val tab: TabLayout.Tab? = parentFragmentManager.fragments[0].view?.findViewById<TabLayout>(R.id.tl_habit)?.getTabAt(2)
                    tab?.select()

                    HabitFragment().refreshFragment(WaitingFragment(), childFragmentManager)
                }
            }
        }
    }
    ...
    override fun onStart() {
        super.onStart()
        Log.e("ProceedFragment", "onStart")
        val types: List<String> = listOf("진행중", "완료요청", "완료대기")
    ...
    companion object {
         ...
         * @return A new instance of fragment ProceedFragment.
         */
        // TODO: Rename and change types and number of parameters
        @JvmStatic
        fun newInstance(param1: String, param2: String) =
            ProceedFragment().apply {
